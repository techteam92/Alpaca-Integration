...
APCA_API_KEY_ID ="PK3REHAIDHGAM1JC7PO6"
APCA_API_SECRET_KEY = "TqyyDze8mmQkopiCFHXo8VRS54o9oCtLqY8bO4g7"
...
def option_info(symbol,update_data, price, side):
    try:
        ...
        filtered = [(symbol, price, qty) for symbol, price, qty in ap_values if (signal_type == "call" and 1 < price < 10) or (signal_type == "put" and 1 < price < 16)]
        if filtered:
            ...
            filtered = [(symbol, selected_price, qty) for symbol, selected_price, qty in ap_values if (signal_type == "call" and 1 < price < 20) or (signal_type == "put" and 1 < price < 30)]
            if filtered:
                selected_symbol, selected_price, selected_qty = random.choice(filtered)
    ...
    while True:
        try:
            ...
            if current_stock_price != last_stock_price:
                ...
                if stoploss is not None and option_price < stoploss:
                    ...
                    alpaca_position_closeAll(option_symbol,current_quantity,opposite_side)
                    print("alpaca position closeAll for stoploss completed")
                    qty = 0
                    break
                if (takeprofit1 is not None and option_price > takeprofit1) and qty == quantity:
                    print("😇takeprofit1 triggered. Closing all positions.🤗")
                    opposite_side = "sell" if ask_type =="buy" else "buy"
